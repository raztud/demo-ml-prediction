...
def main():
    ...
    for idx, url in enumerate(urls):
        url = url.strip()
        s3_file = f"csv_file_{idx}.csv"  # generate a smarter unique and identifiable name
        process = multiprocessing.Process(target=save_csv, args=(url, s3_bucket_name, s3_file))
